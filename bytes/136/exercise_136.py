...
def check_bt(donor, recipient):
    ...
    converted = []
    ## check the input
    accepted = [int, str, Bloodtype]
    for i in [donor, recipient]:
        if type(i) not in accepted:
            raise TypeError("Wrong input type")
        if i in range(0, 8):
            converted.append(i)
        elif i in blood_type_text.keys():
            converted.append(blood_type_text[i].value)
        elif type(i) == Bloodtype:
