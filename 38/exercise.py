...
def get_movies():
    ...
    root = get_tree()
    return [x.attrib['title'] for x in root.findall('./movie')]
...
def get_movie_longest_runtime():
    ...
    root = get_tree()
    max_runtime = max([x.attrib['runtime'] for x in root.findall('./movie')])
    foundit = [x.attrib['title'] for x in root.findall('./movie') if x.attrib['runtime'] == max_runtime]
    return ''.join(foundit)