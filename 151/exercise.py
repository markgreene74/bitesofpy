...
def operas_both_at_premiere(guest, composer):
    ...
    if not (guest in composers.keys() and composer in composers.keys()):
        raise ValueError
    else:
        guest_DOB = _get_date(composers[guest].born)
        guest_DOD = _get_date(composers[guest].died)
        composer_DOB = _get_date(composers[composer].born)
        composer_DOD = _get_date(composers[composer].died)

    # build a list of operas whose author is composer
    operas_list = []
    for i in operas:
        if i.author == composer:
            operas_list.append(i)

    # start checking every opera's date against composer/guest DOB/DOD
    # and make a list to return
    results = []
    for j in operas_list:
        if (guest_DOB < _get_date(j.date) < guest_DOD) and (
            composer_DOB < _get_date(j.date) < composer_DOD
        ):
            results.append(j.play)
