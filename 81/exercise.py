...
tweets = [
    Tweet(
        text=(
            "It's shocking that the vast majority of online banking "
    ...
    Tweet(
        text=(
            "The most unbelievable aspect of the Star Trek universe "
    ...
    Tweet(
        text=(
            "This tutorial has a great line-by-line breakdown of how "
...
def filter_tweets_on_polarity(tweets, keep_positive=True):
    ...
    if keep_positive:
        return [i for i in tweets if i.polarity > 0]
    else:
        return [i for i in tweets if i.polarity < 0]
