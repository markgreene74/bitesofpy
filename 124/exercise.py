...
def most_popular_characters(characters=characters, top=5):
    ...
    for ch in characters:
        _appearances = 0 if ch.appearances == "" else int(ch.appearances)
        c.update({ch.pid: _appearances})
    _pids = [i[0] for i in c.most_common(top)]
    return [ch.name for ch in characters if ch.pid in _pids]
...
def max_and_min_years_new_characters(characters=characters):
    ...
    """
    c = Counter([ch.year for ch in characters if ch.year != ""])
    max_year, _ = c.most_common()[0]
    min_year, _ = c.most_common()[-1]
...
def get_percentage_female_characters(characters=characters):
    ...
    """
    c = Counter([ch.sex for ch in characters if ch.sex != ""])
    total = sum(c.values())
    female = c["Female Characters"]
